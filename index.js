...
bot.on("ready", function() {
    const channel = bot.channels.cache.get("932781783655792690");
      if (!channel) return console.error("The channel does not exist!");
      channel.join().then(connection => {
        
        connection.play(ytdl("https://youtu.be/y7e-GC6oGhg"));
...
bot.on('message', message =>{
  ...
  if(command === 'join'){
    bot.commands.get('join').execute(bot, message, args, Discord);
}
});
...
bot.on("message", async message => {
    ...
    if (command === 'help') {
        const helpEmbed = new MessageEmbed()
            ...
            .setFooter('▲ Illuminati ™ Music Bot')
            .setTimestamp()
